import { useQuery } from "@tanstack/react-query";
import React from "react";
import { userProfileAPI } from "../../APIServices/users/usersAPI";
import Avatar from "./Avatar";

const MyFollowers = () => {
  // Fetch userProfile
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["profile"],
    queryFn: userProfileAPI,
  });

  // Get the user's followers
  const myFollowers = data?.user?.followers;

  return (
    <section className="relative py-20 md:py-32 overflow-hidden bg-gray-900">
      {" "}
      {/* Changed to dark background */}
      <div className="relative container px-4 mx-auto">
        <div className="max-w-7xl mx-auto">
          <div className="max-w-2xl mx-auto mb-20 text-center">
            <h1 className="font-heading text-5xl xs:text-6xl md:text-7xl font-bold text-white mb-8">
              {" "}
              {/* Changed to white text */}
              <span>
                My <span className="text-orange-400">Followers</span>{" "}
                {/* Changed orange to a brighter shade */}
              </span>
            </h1>
            <p className="text-lg text-gray-400 mb-6">
              {" "}
              {/* Changed to light gray */}
              Here are all the people who follow you.
            </p>
            <a
              className="inline-flex py-4 px-6 items-center text-orange-400 font-semibold bg-gray-800 border border-gray-700 hover:border-orange-400 rounded-full transition duration-200"
              href="#"
            >
              View More
            </a>
          </div>
          <div className="flex flex-wrap -mx-4 -mb-8">
            {myFollowers?.map((follower) => {
              return (
                <div
                  className="w-full md:w-1/2 lg:w-1/3 px-4 mb-8"
                  key={follower?._id}
                >
                  {" "}
                  {/* Added key prop */}
                  <div className="max-w-md mx-auto py-10 px-6 text-center bg-gray-800 rounded-md">
                    {" "}
                    {/* Darker card background */}
                    {follower?.profilePicture ? (
                      <img
                        className="w-24 h-24 rounded-full block mb-6 mx-auto"
                        src={follower?.profilePicture}
                        alt={follower?.username} // Added alt text for accessibility
                      />
                    ) : (
                      <Avatar />
                    )}
                    <h5 className="text-2xl font-bold text-white mb-2">
                      {" "}
                      {/* Changed to white text */}
                      {follower?.username}
                    </h5>
                    <span className="block text-orange-400 mb-3">
                      {" "}
                      {/* Changed to a brighter orange */}
                      {follower?.email || "No email"}
                    </span>
                    <div className="flex items-center justify-center"></div>
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </section>
  );
};

export default MyFollowers;
